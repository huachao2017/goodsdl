...
import os
from goods2.dl import dataset as ds
    ...
    for var in slim.get_model_variables():
        ...
        for exclusion in exclusions:
            if var.op.name.startswith(exclusion):
                tf.logging.info('ignore var: ' + var.op.name)
                excluded = True
                break
        if not excluded:
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        # Get the dataset #
        ######################
        dataset = ds.get_split(FLAGS.dataset_split_name, FLAGS.dataset_dir, FLAGS.example_num)
