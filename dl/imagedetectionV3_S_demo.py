...
class ImageDetector:
    ...
    def detect(self, image_instance, step1_min_score_thresh=.5, step2_min_score_thresh=.5):
        ...
        ret = self.do_addition_logic_work(boxes_step1, scores_step1, upcs_step2, scores_step2, image_instance, image_np, step2_min_score_thresh)

        time3 = time.time()
        logger.info('detectV3_S_demo: %s, %d, %.2f, %.2f, %.2f, %.2f' %(image_instance.deviceid, len(ret), time3-time0, time1-time0, time2-time1, time3-time2))
        return ret, time3-time0

    def do_addition_logic_work(self, boxes_step1, scores_step1, upcs_step2, scores_step2, image_instance, image_np, step2_min_score_thresh):
        ret = []
