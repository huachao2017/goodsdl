...
class ImageDetectorFactory:
    ...
    @staticmethod
    def get_static_detector(export1id, export20id, export2_arr=None, export3_arr=None, step2_model_name='nasnet_large'):
        if step2_model_name not in step2_model_names:
            return None
        # step2_model_name : 'nasnet_large','inception_resnet_v2'

        key = '{}_{}'.format(str(export1id),str(export20id))
        if key not in ImageDetectorFactory._detector:
            ImageDetectorFactory._detector[key] = ImageDetector(export1id,export20id,export2_arr,export3_arr,step2_model_name)
        return ImageDetectorFactory._detector[key]

class ImageDetector:
    def __init__(self, export1id, export20id, export2_arr, export3_arr, step2_model_name):
        file_path, _ = os.path.split(os.path.realpath(__file__))
        self.step1_cnn = Step1CNN(os.path.join(file_path, 'model', str(export1id)))
        self.step20_cnn = Step2CNN(os.path.join(file_path, 'model', str(export20id)),step2_model_name)
        self.step2_cnn = Step3CNN(os.path.join(file_path, 'model'), export2_arr)
    ...
    def load(self):
        ...
        if not self.step20_cnn.is_load():
            self.step1_cnn.load(self.config)
            self.step20_cnn.load(self.config)
    ...
    def detect(self, image_instance, step1_min_score_thresh=.5, step2_min_score_thresh=.5):
        ...
        for i in range(boxes.shape[0]):
            if scores[i] > step1_min_score_thresh:
                ...
                step2_images.append(self.step20_cnn.pre_detect(new_image_path))
        ...
        probabilities = self.step20_cnn.detect(step2_images)
    ...
    def do_addition_logic_work(self, boxes_step1, scores_step1, labels_step2, scores_step2, step2_image_paths, image_instance, image_np, step2_min_score_thresh):
        ...
        for i in range(len(boxes_step1)):
            ...
            action = 0
            upc = self.step20_cnn.labels_to_names[class_type]
            if scores_step2[i] < step2_min_score_thresh:
                # 识别度不够
                class_type = -1
                upc = ''
            ...
            # TODO 重新实现后续检测
            # if upc == 'bottled_drink_stand' or upc == 'lack_info_stand':
            #     # 立姿水需要躺倒平放
            #     class_type = -1
            #     upc = ''
            #     action = 2
            # elif upc in self.step20_cnn.cluster_upc_to_traintype:
            #     traintype = self.step20_cnn.cluster_upc_to_traintype[upc]
        ...
        if len(ret) > 0:
            ...
            visualize_boxes_and_labels_on_image_array_V2(
                ...
                scores_step1,
                scores_step2,
                self.step20_cnn.labels_to_names,  # TODO need fix
